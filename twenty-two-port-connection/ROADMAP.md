...
- [x] `twc rename` and `twc add` allow setting a profile name to a reserved CLI subcommand (e.g. `add`, `list`, `remove`, `rename`, `show`, `edit`, `copy`), which makes the profile unreachable via `twc <name>`
